...
TITLE C0nw0nk - Directory Largest File first AV1
...
for /f "delims=" %%i in ('Type "%root_path:"=%%~n0.txt"') do (
	echo "%%i"
	for /f %%b in ('%root_path:"=%win-x64\Tools\FfMpeg\bin\ffmpeg.exe -i "%%i" 2^>^&1 ^| find /c /i "Video: av1 "') do (
		if [%%b]==[0] (
			echo "%%i" is not AV1 encoded
			call "%root_path:"=%PlexCleaner_AV1.cmd" "%%i" "0" "0" "0"
		) else (
			echo "%%i" is AV1 encoded
		)
	)
)
